...
# janitor::clean_names()
univariate = read_excel("Data/Post-COVID Dataset for Yifei_2.xlsx", sheet = "univariate dataset") %>% 
  janitor::clean_names()
secondary_univariate = read_excel("Data/Post-COVID Dataset for Yifei.xlsx", sheet = "secondary univariate dataset") %>% 
  janitor::clean_names()


## Some issues on education variable:
education = primary_univariate = read_excel("Data/Post-COVID Dataset for Yifei.xlsx", sheet = "primary univariate dataset") %>%  
  janitor::clean_names() %>% 
  select(record_id, education)

primary_univariate = univariate %>% 
  select(-education) %>% 
  left_join(education, by = 'record_id') %>% 
  relocate(record_id, age, sex, race, ethnicity, language, education, pmh_neuro, everything())
...
dat = left_join(primary_univariate, secondary_univariate, by = "record_id") %>% 
  ...
  mutate(education = case_when(
    education %in% c(0, 1, 2, 3) ~ 8,
    education %in% c(4, 5, 6, 7) ~ 9)) %>% 
...
covnames = c("age", "sex", "race", "ethnicity", "language_primary", "education", 
             "pmh_neuro", "pmh_enc_is_ich", "pmh_covid_severe", "icu", "icu_los", "hos_los", "steroids",
             "ad8_15", "ecog_16", "ucla", "stress", "sleep", "dsm_depression", 
             "dsm_anxiety", "language_secondary", "vaccine")

outnames = c("memory_cs", "executive_cs", "processing_cs", "verbal_cs", 
             "global_cs", "mrs", "ad8_28", "ecog_29", "pasc_cognition")
